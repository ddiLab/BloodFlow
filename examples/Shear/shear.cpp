...
//    vtkOut.writeData<float>(*computePressure(lattice), "pressure", dx*dx/(dt*dt));
}
...
/*
    ...
    const T uMax = (T) atof(argv[2]);//0.5917e-3;//uMaxRef /(T)N * (T)Nref; // Needed to avoid compressibility errors.
    T Re = uMax*6;//5e-3;
    ...
    plint iSave =500;//2000;//10;
    plint iCheck = 1000000000*iSave;
...
/*
    ...
    /*
    Box3D blockage = Box3D(0,32, 0,40, 23,32);
    defineDynamics(lattice,blockage,new BoundBack<T,DESCRIPTOR>);
    */
    ...
    //Box3D addForce = Box3D(0,nx-1,0,ny-1,nz-2,nz-1);
    //Box3D noForce = Box3D(0,nx-1,0,ny-1,0,nz-1);
    //Array<T,3> force(0,0.,1e-7);
    Array<T,3> force0(0.0,0.0,0.0);
    //setExternalVector(lattice,addForce,DESCRIPTOR<T>::ExternalField::forceBeginsAt,force);
    setExternalVector(lattice,lattice.getBoundingBox(),DESCRIPTOR<T>::ExternalField::forceBeginsAt,force0);
    ...
    for (plint iT=5000;iT<0;iT++){
        lattice.collideAndStream();
//        pcout << "Timestep: " << iT << endl;
...
//    loadBinaryBlock(lattice,"checkpoint.dat");
    writeVTK(lattice,parameters,1);
//    saveBinaryBlock(lattice,"checkpoint.dat");
    pcout << "Eq completed" << endl;
    T timeduration = T();
...
//        Array<T,3> force(0,0.,1e-7);
        Array<T,3> force(0.0,0.0,0.0);
        setExternalVector(lattice,lattice.getBoundingBox(),DESCRIPTOR<T>::ExternalField::forceBeginsAt,force);
//        setExternalVector(lattice,noForce,DESCRIPTOR<T>::ExternalField::forceBeginsAt,force0);
