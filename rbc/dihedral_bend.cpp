...
void DihedralBend::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < ndihedrallist; n++) {
    ...
    s = rg*rabinv*(ax*vb3x + ay*vb3y + az*vb3z);//sin(a), a is the angle between 
    // two faces
/*
      int me1 =  comm->me;
      
      if (me1 == 0){
        printf("angle: c=%g s=%g, equ %g %g %d\n", c,s, cos_shift[type],sin_shift[type],shift[type]);
      }*/
    // error check
    ...
    ddf1 = p*c - df1*s;//cos(a)
    df1 = p*s + df1*c;//sin(a)
    p = ddf1;//cos(a)

    p = p*cos_shift[type] + df1*sin_shift[type];//cos(a-a0)
    df1 = df1*cos_shift[type] - ddf1*sin_shift[type];//sin(a-a0)
    df1 *= -m;//-sin(a-a0)
    p = 1.0 - p;//1-cos(a-a0)

    if (eflag) edihedral = k[type] * p;//kb(1-cos(a-a0))
    ...
    //df = -k[type] * df1;//for type kb(1+cos(a-a0))
    // kb(1-cos(a-a0))
    df = k[type] * df1;
...
void DihedralBend::coeff(int narg, char **arg)
{
  ...
  int shift_one = force->inumeric(FLERR,arg[2]);
  //printf("shift one k=%g %d %d\n",k_one,shift_one,atoi(arg[2]));
  int count = 0;
...
void DihedralBend::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->ndihedraltypes; i++)
    fprintf(fp,"%d %g %d\n",i,k[i],shift[i]);
}
