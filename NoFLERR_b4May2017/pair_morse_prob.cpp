...
PairMorseProb::PairMorseProb(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
...
void PairMorseProb::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (rand_num < prob[itype][jtype]){// attraction probability
          r = sqrt(rsq);
          dr = r - r0[itype][jtype];
          ...
          if(nevery[itype][jtype]){
            if (update->ntimestep % nevery[itype][jtype] == 0){
              if ((type[i] != newType1[itype][jtype]) && (update->ntimestep - q[i] >= nevery[itype][jtype])) {
                ...
                q[i] =static_cast<double>(update->ntimestep);
              }
              if ((type[j] != newType2[itype][jtype]) && (update->ntimestep - q[j] >= nevery[itype][jtype])) {
                //printf("id %d type %d %d prob %g rand %g charge %g %g nevery %d \n",j, itype, jtype, prob[itype][jtype],rand_num, q[i], q[j], nevery[itype][jtype]);
                type[j]=newType2[itype][jtype];
                q[j] =static_cast<double>(update->ntimestep);
              }
            }
          }//eof atom type
...
void PairMorseProb::allocate()
{
  ...
  memory->create(prob,n+1,n+1,"pair:prob");
...
void PairMorseProb::settings(int narg, char **arg)
{
...
void PairMorseProb::coeff(int narg, char **arg)
{
  ...
  double prob_one = 1.0;//force->numeric(FLERR,arg[7]);
  ...
  while (iarg<narg){
    if (strcmp(arg[iarg],"nevery")==0){
      if (iarg+4>narg) error->all(FLERR,"Illegal pair_style morse/prob command");
      neveryFlag=1;
      ...
      if (iarg+2>narg) error->all(FLERR,"Illegal pair_style morse/prob command");
      probFlag=1;
      prob_one = force->numeric(FLERR,arg[iarg+1]);
      iarg += 2;
    }
  }
...
double PairMorseProb::init_one(int i, int j)
{
...
void PairMorseProb::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        ...
        fwrite(&prob[i][j],sizeof(double),1,fp);
...
void PairMorseProb::read_restart(FILE *fp)
{
    ...
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          ...
          fread(&prob[i][j],sizeof(double),1,fp);
        ...
        MPI_Bcast(&prob[i][j],1,MPI_DOUBLE,0,world);
...
void PairMorseProb::write_restart_settings(FILE *fp)
{
...
void PairMorseProb::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairMorseProb::write_data(FILE *fp)
{
...
void PairMorseProb::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairMorseProb::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairMorseProb::extract(const char *str, int &dim)
{
  dim = 2;
