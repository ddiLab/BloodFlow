  ...
  Fix(lmp, narg, arg)
{
  ...
  if(narg <7) error->all(FLERR,"Illegal fix lbm command");
  ...
  nevery = atoi(arg[3]);
  typeLB = atoi(arg[4]);
  viscosity = atof(arg[5]);//can get tau
  ...
  while (iarg < narg){
	...
	for(int i=0; i<=atom->ntypes; i++) NodeArea[i] = -1.0;
      ...
      if(setArea == 1)
	error->all(FLERR,"Illeg fix lb/fluid command: cannot use a combination of default and user-specified gamma values");
      setGamma = 1;
      double Gammaone;
...
a z wall velocity without implementing fixed BCs in z");//jifu
 ...
 //count: # of blocks      blockLength: # elemts each block
 //stride:# elemts between start of each block
  MPI_Type_vector(subNbz-2,numvel,numvel,MPI_DOUBLE,&oneslice);
  MPI_Type_commit(&oneslice);
  //mpi_type_create_hvector(count,blockLength,stride,oldtype,newtype)
  //stride:# of bytes between start of each block
  ...
  if(printfluid > 0){
    ...
    memory->create(buf,subNbxmax,subNbymax,subNbzmax,4,"FixLBM:buf");
    if(me==0)
...
void FixLBM::init(void)
{
  ...
  if(comm->me ==0) printLBMInfor();
}

void FixLBM::printLBMInfor(void){
  printf("----------------Lattice Boltzmann Fluid----------------------\n");
  printf("LB grid of %d by %d by %d\n",Nbx,Nby,Nbz);
  printf("dx = %f, dt = %f, dm = %f, tau = %f \n",dx_lb,dt_lb,dm_lb,tau);
  printf("-------------------------------------------------------------\n");
}
...
void FixLBM::read_restartfile(void)
{
  ...
  if( comm->myloc[2]==comm->procgrid[2]-1) starts[2] = comm->myloc[2]*(subNbz-3);//jifu
  if( comm->myloc[0]==comm->procgrid[0]-1) starts[0] = comm->myloc[0]*(subNbx-3);
  if( comm->myloc[1]==comm->procgrid[1]-1) starts[1] = comm->myloc[1]*(subNby-3);
...
void FixLBM::update_full15(void)
{
   ...
   if(domain->periodicity[2]==0){  
    ...
    //  (this->*(pbc[nbc].bcAction))(pbc[nbc].index);
...
void FixLBM::preLow(int idx){}
void FixLBM::preHigh(int idx){}
void FixLBM::wallLow(int idx){
