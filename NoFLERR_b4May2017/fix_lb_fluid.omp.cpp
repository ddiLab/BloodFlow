...
void FixLbFluid::peskin_interpolation(int i)
{
  ...
  //--------------------------------------------------------------------------
  #pragma omp parallel for private(rsq,r,weightx,weighty,weightz,ixp,iyp,izp,ii,jj,kk) shared(k,unode,mnode) 
  for(ii=-1; ii<3; ii++){
    rsq=(-dx1+ii)*(-dx1+ii);
...
require more frequent neighborlist rebuilds");
	...
	//FfP[isten] = weightx*weighty*weightz;i
        int idx=(ii+1)*16+(jj+1)*4+kk+1;
	FfP[idx] = weightx*weighty*weightz;
	// interpolated velocity based on delta function.
        //#pragma omp critical
	for(k=0; k<3; k++){
	  //unode[k] += u_lb[ixp][iyp][izp][k]*FfP[isten];
          #pragma omp atomic
	  unode[k] += u_lb[ixp][iyp][izp][k]*FfP[idx];
	}
        if(setGamma==0){
	  //mnode += density_lb[ixp][iyp][izp]*FfP[isten];
          #pragma omp atomic 
	  mnode += density_lb[ixp][iyp][izp]*FfP[idx];
        }
*/	
	//isten++;
      }
